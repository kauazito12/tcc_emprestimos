...
function Professores() {
  ...
  async function buscarProfessores() {
    try {
      ...
      if (!resposta.ok) {
        mostrarMensagem(dados.erro || "Erro ao buscar professores", "erro");
        return;
      }
      ...
      console.error(error);
      mostrarMensagem("Erro ao buscar professores", "erro");
    }
  }
  ...
  async function salvarProfessor(e) {
    ...
    try {
      ...
      if (editandoId) {
        resposta = await fetch(`${API_URL}/professores/${editandoId}`, {
          method: "PUT",
          headers: {
        ...
        resposta = await fetch(`${API_URL}/professores`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
      ...
      if (!resposta.ok) {
        mostrarMensagem(dados.erro || "Erro ao salvar professor", "erro");
        return;
      }
      ...
      console.error(error);
      mostrarMensagem("Erro ao salvar professor", "erro");
    }
  }
  ...
  async function excluirProfessor(id) {
    try {
      ...
      if (!resposta.ok) {
        mostrarMensagem(dados.erro || "Erro ao excluir professor", "erro");
        return;
      }
      ...
      console.error(error);
      mostrarMensagem("Erro ao excluir professor", "erro");
    }
  }
    ...
    <div className="professores-page">
      ...
      <div className="professores-card">
        ...
        <form className="professores-form" onSubmit={salvarProfessor}>
          ...
          <div className="form-acoes">
            <button type="submit">
              {editandoId ? "Atualizar Professor" : "Salvar Professor"}
            </button>
      ...
      <div className="professores-card">
        ...
        {professores.length === 0 ? (
          <p className="texto-vazio">Nenhum professor cadastrado.</p>
        ) : (
                  ...
                  <tr key={professor.id}>
                    ...
                    <td>
                      <div className="acoes-linha">
                        <button onClick={() => editarProfessor(professor)}>
                          Editar
                        </button>
                        <button
